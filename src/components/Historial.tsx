...
import { History, User as UserIcon, Tag, Calendar, ChevronRight, Image as ImageIcon, FileSpreadsheet, FileText as FilePdfIcon } from 'lucide-react';
                ...
                >
                  {userHistory[selectedUser].exports.map((log, idx) => (
                    <motion.div
                      ...
                      transition={{ delay: idx * 0.05 }}
                      className="bg-white p-4 rounded-xl border border-gray-100 flex items-center justify-between group shadow-sm hover:border-green-200"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${log.tipo_formato === 'EXCEL' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                          {log.tipo_formato === 'EXCEL' ? <FileSpreadsheet size={16} /> : <FilePdfIcon size={16} />}
                        </div>
                        <div>
                           <p className="text-[11px] font-bold text-[#1F3864] uppercase tracking-tight">Exportado: {log.tagname}</p>
                           <p className="text-[9px] text-gray-400">{new Date(log.timestamp).toLocaleString()}</p>
                        </div>
                      </div>
                      <div className="text-[9px] font-bold text-gray-300 uppercase tracking-widest px-2 py-1 bg-gray-50 rounded border border-gray-100">
                        {log.tipo_formato}
                      </div>
                    </motion.div>
                  ))}
                  {userHistory[selectedUser].exports.length === 0 && (
                    <div className="text-center p-12 text-gray-400 text-sm">No hay backups de exportaciones aún.</div>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
